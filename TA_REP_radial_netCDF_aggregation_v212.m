%% TA_REP_radial_netCDF_aggregation_v212.m
...
function [rnA_err] = TA_REP_radial_netCDF_aggregation_v212(networkID,vers,radSite,folders,initialDate,finalDate,perMonth)

%% Setup

disp(['[' datestr(now) '] - - ' 'TA_REP_radial_netCDF_aggregation_v212.m started.']);
...
if(rnA_err==0)
    disp(['[' datestr(now) '] - - ' 'TA_REP_radial_netCDF_aggregation_v212 successfully executed.']);
else
    disp(['[' datestr(now) '] - - ' 'TA_REP_radial_netCDF_aggregation_v212 exited with an error.']);
end
